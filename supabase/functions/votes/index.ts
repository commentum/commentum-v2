...
serve(async (req) => {
  ...
  ;(async () => {
    try {
      // Calculate vote changes for the comment author
      let upvoteChange = 0
      let downvoteChange = 0
      
      if (vote_type === 'upvote') {
        if (currentVote === 'upvote') {
          upvoteChange = -1 // removed upvote
        } else if (currentVote === 'downvote') {
          upvoteChange = 1  // changed to upvote
          downvoteChange = -1 // removed downvote
        } else {
          upvoteChange = 1  // new upvote
        }
      } else if (vote_type === 'downvote') {
        if (currentVote === 'downvote') {
          downvoteChange = -1 // removed downvote
        } else if (currentVote === 'upvote') {
          downvoteChange = 1  // changed to downvote
          upvoteChange = -1   // removed upvote
        } else {
          downvoteChange = 1  // new downvote
        }
      } else if (vote_type === 'remove') {
        if (currentVote === 'upvote') {
          upvoteChange = -1 // removed upvote
      ...
      // Update the comment author's statistics
      if (upvoteChange !== 0 || downvoteChange !== 0) {
        // Get current user stats
        const { data: currentUser } = await supabase
          .from('users')
          .select('total_upvotes_received, total_downvotes_received')
        ...
        if (currentUser) {
          ...
          const newUpvotes = Math.max(0, (currentUser.total_upvotes_received || 0) + upvoteChange)
          const newDownvotes = Math.max(0, (currentUser.total_downvotes_received || 0) + downvoteChange)

          // Update user stats
          await supabase
            .from('users')
            .update({
              total_upvotes_received: newUpvotes,
              total_downvotes_received: newDownvotes,
      ...
      console.error('Failed to update user vote statistics (background):', error)
      // Silently fail - don't impact voting
    }
